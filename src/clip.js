    ...
    Object.keys(features).forEach(function (featureId) {
        for (var i = 0; i < features[featureId].length; i++) {
            ...
            if (min >= k1 && max <= k2) { // trivial accept
                clipped[featureId] = clipped[featureId] || [];
                clipped[featureId].push(feature);
                length++;
                continue;
            ...
            if (slices.length) {
                ...
                clipped[featureId] = clipped[featureId] || [];
                clipped[featureId].push(createFeature(feature.tags, type, slices, feature.id));
                length++;
            }
        }
    });

    return length ? clipped : null;
}
